# Project Notes for Claude

## Random Number Generation

Random operations are assumed to be expensive. Where multiple random bytes of known length are needed in short succession (e.g., a 16-byte vec and possibly a single `u8`), a 17-byte stack-allocated buffer should be used, with copies taken from this buffer rather than making separate RNG calls.

## Code Generation

- **Do not manually edit generated files.** All files in `radius/src/dict/` with the header `// Code generated by machine generator; DO NOT EDIT.` are generated from `radius/dicts/` by the `code-generator` crate.
- Trigger regeneration by touching `radius/build.rs` then running `cargo build --package radius`. The `build.rs` calls `code_generator::generate(&out_dir, &dict_file_path_refs)` on every dict file in `radius/dicts/`.
- One `.rs` file is emitted per dictionary file. The generator also runs `rustfmt --edition=2021` on its output internally.
- **Standard RFC attributes** (no `BEGIN-VENDOR` block) get: an `{ATTR}_TYPE: AVPType` constant, plus `add_*`, `delete_*`, `lookup_*`, and `lookup_all_*` helpers on `Packet`.
- **Vendor-Specific Attributes (VSA)** inside `BEGIN-VENDOR`/`END-VENDOR` blocks get: an `{ATTR}_VENDOR_TYPE: u8` constant with the vendor sub-type, plus `add_*`, `delete_*`, `lookup_*`, and `lookup_all_*` helpers that use `AVP::from_vsa` / `Packet::lookup_vsa` / `Packet::delete_vsa` to encode/decode properly as RADIUS type-26 Vendor-Specific AVPs.
- Attribute names are transformed via the `inflector` crate: `snake_case` for methods, `PascalCase` for types, `SCREAMING_SNAKE_CASE` for value constants.
- Sub-attributes with dotted type IDs (e.g. `153.2`) are silently skipped.
- Duplicate attribute type numbers within the same vendor scope are silently skipped.

## Crate Structure

- **`radius`** — the main library crate; exposes `client`, `server`, and `core`.
- **`code-generator`** — internal binary; not published; reads dict files and writes source.
- **`examples`** — separate crate with `client.rs` and `server.rs` example binaries.
- Workspace resolver is `"2"`; MSRV is **Rust 1.75**.

## Crypto Backends

Exactly one of three mutually exclusive features must be active (enforced via `compile_error!` in `lib.rs`):

- **`aws-lc`** (default) — uses `aws-lc-sys` FFI directly; `RAND_bytes`, `MD4`, `SHA-1`, and `DES-ECB` from AWS-LC.
- **`openssl`** — uses the `openssl` and `rand` crates.
- **`rust-crypto`** — pure-Rust backend using RustCrypto crates (`md4`, `sha1`, `des`) + `rand`.

**MD5 and HMAC-MD5 are unconditionally provided by the in-tree `fast_md5` module** regardless of which backend is active. The backend features only control: `fill_random`/`random_bytes` (RNG) and the `md4`/`sha1`/`des_ecb_encrypt` primitives used by the MS-CHAP helpers.

The unified API in `radius/src/core/crypto.rs` exposes: `md5(data) -> [u8; 16]`, `md5_of(parts) -> [u8; 16]`, `hmac_md5(key, data) -> [u8; 16]`, `random_bytes(n) -> Box<[u8]>`, `fill_random(buf: &mut [u8])`.

To test all backends: `make check_all` (also `make check_openssl`, `make check_rust_crypto`).

## Key Abstractions

- **`Packet`** — core struct (`code`, `identifier`, `authenticator`, `secret`, `attributes`). Max wire size 4096 bytes. `Packet::new` picks a random identifier; use `Packet::new_with_identifier` when you need a specific one. `Packet::decode` parses wire bytes.
- **`AVP`** — `{ raw: Bytes }` (wire encoding: type byte + length byte + value). Constructors/decoders for every RADIUS value type including `from_user_password` and `from_tunnel_password` (encrypted variants). Buffer-reuse `_in` variants accept a `&mut BytesMut` arena for packet-building.
- **`Request`** — wraps an incoming `Packet` with `local_addr` and `remote_addr`.
- **`Client`** — UDP; configurable connection/socket timeouts; no built-in retransmission.
- **`Server`** — async Tokio UDP server; requires a `RequestHandler` and `SecretProvider`; deduplicates in-flight requests via `Arc<RwLock<HashSet<RequestKey>>>`.

## In-Tree MD5 (`radius/src/core/fast_md5/`)

`fast_md5` is a module directory (`mod.rs`, `aarch64.rs`, `x86_64.rs`, `fallback.rs`) implementing a highly-optimised MD5. It is the **primary and unconditional** MD5 implementation: `crypto::md5`, `crypto::md5_of`, and `crypto::hmac_md5` all delegate to it regardless of which backend feature is active.

**Architecture dispatch:**
- `x86_64` — Two **monolithic `asm!` blocks** (one for the 16 F rounds, one for G+H+I rounds), ported from `md5_block_noleag()` in animetosho/md5-optimisation. `NoLEA` replaces each 3-operand `leal K(A,input),A` (3-cycle latency) with `add $K,A` + `add [m+off],D` (two 1-cycle ADDs). `GOpt` splits G into `(~D&C)+(D&B)` to break the B-dependency chain. A rolling TMP1 register carries the prior-round C copy, and each round pre-loads the next input word into the current D register.
- `aarch64` — Rust `f!`/`g!`/`h!`/`i!` round macros, each with a **per-round `asm!` rotate barrier** that pins the accumulator to a concrete 32-bit register at every round boundary, guiding LLVM register allocation (~2% faster than `rotate_left()` alone on Apple Silicon). LLVM naturally emits `BIC`/`ORN` for `!d & c` (G) and `b | !d` (I). Loads use `core::array::from_fn` over a raw pointer so LLVM emits LDP pairs.
- Fallback — Pure Rust with `rotate_left`; same GOpt/I-optimisation forms; LLVM emits `rorxl`/`rorl` as appropriate.

**Performance vs aws-lc on Apple Silicon (measured):**

| Input | fast_md5 | aws-lc |
|-------|----------|--------|
| 16 B  | ~291 MiB/s | ~284 MiB/s (+2.5%) |
| 64 B  | ~510 MiB/s | ~513 MiB/s (≈tie) |
| 256 B | ~776 MiB/s | ~782 MiB/s (-0.8%) |
| 1024 B| ~892 MiB/s | ~900 MiB/s (-0.9%) |
| 16 KB | ~935 MiB/s | ~945 MiB/s (-1.1%) |

The residual ~1% gap at large inputs is structural: aws-lc is a standalone function with an explicit block loop (amortises callee-save overhead over many blocks and builds all 64 constants inline via `movz/movk`), while our function is always-inlined and loads constants from the literal pool.

**Key implementation notes:**
- `struct Md5` (private) — streaming context: `state: [u32; 4]`, `count: u64`, `buf: [u8; 64]`, `buf_len: usize`.
- `pub fn md5(data: &[u8]) -> [u8; 16]` (#[inline]) and `pub fn md5_of(parts: &[&[u8]]) -> [u8; 16]` (#[inline], scatter-gather, no allocation).
- `kp(lo, hi) -> u64` — aarch64-only `const fn` (`#[allow(dead_code)]`) reserved for a future monolithic-asm variant that packs constant pairs as 64-bit operands loaded with `LDP + LSR #32`.

## Conventions

- All error types use `thiserror` and implement `PartialEq` + `Debug`.
- `Bytes`/`BytesMut` (from the `bytes` crate) are used for all binary payloads throughout.
- `#[must_use]` is applied to all getters and lookup functions; the `clippy::double_must_use` lint is suppressed in generated files because lookups return `Option<Result<T, AVPError>>`.
- There are no public re-exports at the crate root — callers import `radius::core::rfc2865`, `radius::client::Client`, etc. directly.
