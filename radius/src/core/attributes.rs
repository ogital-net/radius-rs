...
impl Attributes {
    ...
    /// Iterator-returning variant of [`Attributes::lookup_all`] that does not
    /// allocate an intermediate `Vec<&AVP>`.
    #[inline]
    pub(crate) fn lookup_all_iter(&self, typ: AVPType) -> impl Iterator<Item = &AVP> + '_ {
        self.0.iter().filter(move |avp| avp.typ() == typ)
    }
