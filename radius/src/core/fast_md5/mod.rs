...
//!   Input words are loaded on demand via a closure (not front-loaded) so
//!   LLVM can interleave LDPs with round arithmetic.
...
// MD5/HMAC-MD5 constants come straight from RFC 1321 / RFC 2104; rendering them
// with `_` separators would obscure the spec match.  `k_ipad` / `k_opad` are
// likewise the standard RFC 2104 names.
...
/// Scatter-gather form of [`hmac_md5`]: HMAC-MD5 over the concatenation of
/// `parts`, without allocating an intermediate buffer.  Equivalent to
/// `hmac_md5(key, &parts.concat())`.
#[must_use]
...
/// Streaming HMAC-MD5 (RFC 2104) context.
///
/// Allows callers to feed input piecewise without first materializing the
/// concatenated message in memory.  All state is stack-allocated; no heap
/// activity occurs after construction.
pub struct HmacMd5 {
    /// Running MD5 over `k_ipad || data...`.
    inner: Md5,
    /// Outer padded key, retained for the final `MD5(k_opad || inner_hash)` step.
    k_opad: [u8; HMAC_BLOCK_SIZE],
}

impl HmacMd5 {
    /// Begin a new HMAC-MD5 computation keyed with `key`.
    ///
    /// Keys longer than 64 bytes are pre-hashed with MD5 per RFC 2104.
    #[must_use]
...
mod tests {
    ...
    fn rfc1321_vectors() {
        ...
        for (input, expected) in cases {
            ...
            assert_eq!(hex(&digest), *expected, "input: {input:?}");
        }
    }
    ...
    fn hash_with(data: &[u8], compress: fn(&mut [u32; 4], &[u8; 64])) -> [u8; 16] {
        ...
        if buf_len > 0 {
            ...
            if buf_len == 64 {
                let block: &[u8; 64] = unsafe { &*(buf.as_ptr().cast::<[u8; 64]>()) };
                compress(&mut state, block);
                buf_len = 0;
            }
        }
        while remaining.len() >= 64 {
            let block: &[u8; 64] = unsafe { &*(remaining.as_ptr().cast::<[u8; 64]>()) };
            compress(&mut state, block);
    ...
    fn fallback_rfc1321_vectors() {
        ...
        for (input, expected) in cases {
            ...
            assert_eq!(hex(&digest), *expected, "fallback: input {input:?}");
        }
    }
