...
// • On-demand input loads via a closure: LLVM emits LDPs interleaved with
//   round arithmetic instead of front-loading all 16 words.  Front-loading
//   forces all 8 LDPs to retire before round 1 starts, serializing the
//   load-use latency.  Per-round closure calls let LLVM schedule each LDP
//   adjacent to the round that first uses its result (~0.7% measured win
//   on Apple Silicon vs. an `[u32; 16]` array built via `core::array::from_fn`).
// ─────────────────────────────────────────────────────────────────────────────
